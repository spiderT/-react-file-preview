...
export default function processPptx(setOnMessage = () => {}, postMessage) {
  ...
  async function genShape(node, slideLayoutSpNode, slideMasterSpNode, id, name, idx, type, order, warpObj) {
    ...
    if (shapType !== undefined || custShapType !== undefined) {
      ...
      if (
        (headEndNodeAttrs !== undefined && (headEndNodeAttrs['type'] === 'triangle' || headEndNodeAttrs['type'] === 'arrow')) ||
    ...
    if (shapType !== undefined && custShapType === undefined) {
      ...
      if (node['p:txBody'] !== undefined) {
        result += await genTextBody(node['p:txBody'], slideLayoutSpNode, slideMasterSpNode, type, warpObj);
      }
      result += '</div>';
  ...
  async function genBuChar(node, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    ...
    if (buType === 'TYPE_BULLET') {
      if (buFontAttrs !== undefined) {
        ...
        bullet =
          '<span style=\'font-family: ' + typeface + '; margin-left: ' + marginLeft * lvl + 'px' + '; margin-right: ' + marginRight + 'px' + ';color:' + bultColor + ';font-size:' + bultSize + ';';
        if (isRTL) {
  ...
  async function genChart(node, warpObj) {
    ...
    const xfrmNode = getTextByPathList(node, ['p:xfrm']);
    const result =
      '<div id=\'chart' + chartID + '\' class=\'block content\' style=\'' + getPosition(xfrmNode, undefined, undefined) + getSize(xfrmNode, undefined, undefined) + ' z-index: ' + order + ';\'></div>';
